...
public class ServicePayU implements IServicePayU {
	...
	public CreditCardToken tokenizar(SolicitudToken solicitud)
		...
		parameters.put(PayU.PARAMETERS.CREDIT_CARD_NUMBER, solicitud.getCardData().getCardNumber());
		// Ingrese aquí la fecha de vencimiento de la tarjeta de crédito
		parameters.put(PayU.PARAMETERS.CREDIT_CARD_EXPIRATION_DATE, solicitud.getCardData().getCardExpirationDate());
		// Ingrese aquí el nombre de la tarjeta de crédito
		parameters.put(PayU.PARAMETERS.PAYMENT_METHOD, solicitud.getCardData().getPaymentMethod());
	...
	public void eliminarToken(Token token) throws PayUException, InvalidParametersException, ConnectionException {
		...
		parameters.put(PayU.PARAMETERS.TOKEN_ID, "d6ae0ceb-9a16-4318-a70d-493166ab26ca");
		CreditCardToken response = PayUTokens.remove(parameters);
	
	}
	
	@Override
	public void pagarConToken(Customer customer, Token token) throws PayUException, InvalidParametersException, ConnectionException {
		String reference = "payment_test_000000012";
		String value= "10000";
		...
		parameters.put(PayU.PARAMETERS.TOKEN_ID, "828ddab3-6316-4dbb-8e10-f0c93f05d617");
	...
	public void pagarSinToken() throws PayUException, InvalidParametersException, ConnectionException {
		...
		//Ingrese aquí el número de la tarjeta de crédito
		parameters.put(PayU.PARAMETERS.CREDIT_CARD_NUMBER, "4916972070125");
		//Ingrese aquí la fecha de vencimiento de la tarjeta de crédito
		parameters.put(PayU.PARAMETERS.CREDIT_CARD_EXPIRATION_DATE, "2023/06");
		//Ingrese aquí el código de seguridad de la tarjeta de crédito
		parameters.put(PayU.PARAMETERS.CREDIT_CARD_SECURITY_CODE, "659");
		//Ingrese aquí el nombre de la tarjeta de crédito
		...
		logger.info(response.getAuthorizationCode());
		logger.info(response.getOrderId()+ "");
		logger.info(response.getPaymentNetworkResponseCode());
		logger.info(response.getPaymentNetworkResponseErrorMessage());
		logger.info(response.getState().name());
		logger.info(response.getAdditionalInfo().toString());
		logger.info(response.getResponseCode().name());
